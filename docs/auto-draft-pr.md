...
`ubuntu-latest`. The PowerShell `pwsh` shell ships preinstalled. Requires PowerShell 7.4+ for native-command exit-code propagation via `$PSNativeCommandUseErrorActionPreference`.
